...
func (a *app) Ensure(config caas.ApplicationConfig) (err error) {
	...
	var configureStorage = func(storageUniqueID string, handlePVC handlePVCFunc, handleVolumeAttachment handleVolumeAttachmentFunc) error {
		err := a.configureStorage(
			storageUniqueID,
			config.Filesystems,
			config.FilesystemUnitAttachments,
			storageClasses,
			handleVolume, handleVolumeMount, handlePVC, handleVolumeAttachment, handleStorageClass,
		)
		return errors.Trace(err)
	}
	...
	case caas.DeploymentStateful:
		...
		if err = configureStorage(
			...
			func(pvc corev1.PersistentVolumeClaim, pvcName string, volumeName string) error {
				...
				if err := persistentVolumeClaim.Validate(nil, utils.LabelsJuju); err != nil {
					return errors.Trace(err)
				}
				return nil
			},
...
type handleVolumeAttachmentFunc func(pvc corev1.PersistentVolumeClaim, pvcName string, volumeName string) error
...
func (a *app) configureStorage(
	...
	handleVolumeAttachment handleVolumeAttachmentFunc,
	...
	for index, fs := range filesystems {
		...
		if unitAttachments, ok := filesystemUnitAttachments[fs.StorageName]; ok {
			for _, attachment := range unitAttachments {
				...
				if err = handleVolumeAttachment(*pvc, pvcName, attachment.VolumeId); err != nil {
					return errors.Trace(err)
				}
			}
		}
