test_deploy_attach_storage() {
	...
	# Make sure pvc uid have been update in filesystems.
	PVC_UID=$(
		kubectl get pvc -n "${model_name}" "${NEW_PVC}" -o json |
			jq -r '.metadata.uid'
	)
	OUT=$(
		juju storage --format json | jq '.filesystems."1"."provider-id"'
	)
	echo "${OUT}" | check "${PVC_UID}"

	# Make sure pv name have been update in volumes.
	OUT=$(
		juju storage --format json | jq '.volumes."1"."provider-id"'
	)
	echo "${OUT}" | check "${PV}"
